...
v2ray_install() {
    ...
    mkdir -p /root/v2ray
    cd /root/v2ray || exit
    wget -N --no-check-certificate https://raw.githubusercontent.com/Punl/V2Ray_ws-tls_bash_onekey/master/go.sh
...
nginx_install() {
    ...
    judge "Nginx 下载"
    wget -nc --no-check-certificate https://github.com/Punl/V2Ray_ws-tls_bash_onekey/raw/master/openssl-1.1.1f.tar.gz -P ${nginx_openssl_src}
    judge "openssl 下载"
...
ssl_install() {
    ...
    curl https://github.com/Neilpang/acme.sh | sh
    judge "安装 SSL 证书生成脚本"
}
domain_check() {
